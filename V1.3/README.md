...
| ------------ | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Time to send | timeInterval              | Time (in minutes) between each sending of measurements. <br> Default value = 15 minutes.                                                                                                                                            |
...
| Led Configuration | ledConfiguration | Led indicator configuration. <br> Default value = 0. (Ventilation indicator) <br> <br> 0 = Ventilation indicator <br> 1 = Confort indicator <br> 2 = Temperature indicator <br> 3 = Humidity indicator <br> 4 = CO₂ indicator <br> 5 = VOCS indicator <br> 6 = PM2.5 indicator <br> 7 = PM10 indicator <br> 8 = Virus indicator <br> 9 = IAQ indicator <br> 10 = PM1.0 indicator <br> 11 = PM4 indicator <br> 12 = CH₂O Indicator <br> 13 = O₃ indicator <br> 14 = NO₂ indicator <br> 15 = CO indicator |
Touch Status | touchEnable | Touch status (On or Off). <br> Default value = true. |

> [!NOTE]
> Led configuration depends on the model.
...
| Noise | dB | 30 - 120 dB | **Model with Noise exclusive** <br> Noise value in dB <br> <a href="https://www.inbiot.es/es/wikinbiot/ruido"><img src="https://img.shields.io/badge/Noise information-blue?style=for-the-badge" alt="Link"/></a>                                                                                                                          |
...
The encoder is used to encode downlink messages for the MICA device. It supports multiple platforms, including Chirpstack, The Things Network, and Milesight. The encoder translates configuration parameters into byte arrays that the device can interpret.

## Supported Platforms

- **Chirpstack v4**: Uses the `encodeDownlink` function.
- **Chirpstack v3 and Milesight**: Uses the `Encode` function.
- **The Things Network**: Uses the `Encoder` function.

## Main Encoder Function

The main function, `inbiotDeviceDecode`, processes the input payload and encodes it into a byte array. It supports the following parameters:
...
| `timeToSend`   | Sets the periodicity of data sending in minutes. Range: `5-60`.     <br> Default configuration = 15 min (value 0)                            | `{ "timeToSend": 10 }` |
...
> [!NOTE]
>Take into account that if the led parameter is set to a value that is not supported by the model, the change will not be applied.
